...
#my_db <- src_sqlite("Data/VMT.sqlite3", create = T)
...
## Set extent
#nex <- extent(2020000, 2140000, -4100000, -3950000) #CoCT
nex <- extent(2030000, 2060000, -4080000, -4010000) #Peninsula?
...
lc3 <- subs(lc, , by = "ID", which = "LC3V") #



#Select minimum required data

if(!exists("Data/lc2class.grd"))
{
lcx <- levels(lc)[[1]]
lc3 <- subs(lc, lcx, by = "ID", which = "LC3V", filename = "Data/lc2class.grd")
}

lc3 <- raster("Data/lc2class.grd")



###

lc3 <- deratify(lc, att="LC3V", complete=TRUE, drop=TRUE) 
pa2 <- deratify(pa, att="TYPE2", layer=1, complete=TRUE, drop=TRUE) 

all <- stack(vm,lc,pa)

names(all) <- c("VM", "LC", "PA")

writeRaster(all, "Data/VMT.tif")
